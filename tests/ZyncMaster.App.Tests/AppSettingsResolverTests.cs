...
public class AppSettingsResolverTests
{
    ...
    [Fact]
    public void Paste_panel_opacity_defaults_to_seventy()
    {
        var resolver = new AppSettingsResolver();

        // A fresh POCO ships 70; the resolver passes it through unchanged.
        new AppSettings().PastePanelOpacity.Should().Be(70);
        resolver.Resolve(Valid()).PastePanelOpacity.Should().Be(70);
    }
    ...
    [InlineData(101, 100)]
    [InlineData(5000, 100)]
    public void Clamps_paste_panel_opacity_into_zero_to_hundred(int input, int expected)
    {
        var resolver = new AppSettingsResolver();

        var engine = resolver.Resolve(new AppSettings { ServerBaseUrl = "https://x", PastePanelOpacity = input });

        engine.PastePanelOpacity.Should().Be(expected);
    }

    [Fact]
    public void Default_server_base_url_is_production()
    {
        // A fresh AppSettings (as generated by LoadOrCreateDefault) ships pointing at prod,
        // so a released install works without hand-editing settings.json.
        new AppSettings().ServerBaseUrl.Should().Be(AppSettings.ProductionServerBaseUrl);

        var resolver = new AppSettingsResolver();
        var engine = resolver.Resolve(new AppSettings());

        engine.ServerBaseUrl.Should().Be(AppSettings.ProductionServerBaseUrl);
    }

    [Fact]
    public void Environment_variable_overrides_settings_file()
    {
        const string envVar = "ZYNCMASTER_SERVER_URL";
        var previous = Environment.GetEnvironmentVariable(envVar);
        try
        {
            Environment.SetEnvironmentVariable(envVar, "http://localhost:59822");
            var resolver = new AppSettingsResolver();

            var engine = resolver.Resolve(new AppSettings { ServerBaseUrl = "https://prod.example" });

            engine.ServerBaseUrl.Should().Be("http://localhost:59822");
        }
        finally
        {
