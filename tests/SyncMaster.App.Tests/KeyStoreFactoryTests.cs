...
public class KeyStoreFactoryTests
{
    ...
    private static string TempPath() =>
        Path.Combine(Path.GetTempPath(), "syncmaster_keystore_" + Guid.NewGuid().ToString("N"), "device.key");
    ...
    public async Task Keychain_round_trip_on_mac_only()
    {
        ...
        var store = new KeychainDeviceKeyStore("SyncMasterTest", "device-api-key-test");
        try
        {
